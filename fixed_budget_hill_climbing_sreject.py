...
Algorithm: hill climbing (for concave graphs), see our paper for details
...
parser = argparse.ArgumentParser(description='FIXED BUDGET HILL CLIMBING SUCCESSIVE REJECT')
parser.add_argument('--filename', type=str, default='synthetic_graph_r100.pkl', metavar='F',
    ...
    for k in range(1, n_arms):  # phase k <- 1 ... n_arms - 1
        ...
        if args.verbose > 2:  # print log
            print('  Phase #{}: n_samples={}, pull_budget_per_arm={} reject_node={}, active_nodes: len={} {}'.format(
                k, n_samples, pull_budget, reject_node, len(arms), '' if len(arms) > 10 else arms))
...
def climb(budget):
    ...
    for p in range(args.max_path_len):
        ...
        path.append(z)  # could be z==x, indicating x is maximum, that's fine, we continue to confirm x
        x = z
    return path
...
print('HILL CLIMBING SUCCESSIVE REJECT STATISTICS:   budget = {}   n_trials = {}'.format(
...
result = {'algorithm': 'hill-climb', 'args': args, 'run-time': elapsed_time,
          'subopt-gaps-mean': gaps.mean(), 'subopt-gaps-std': gaps.std(), 'rank-stats': rank_stats}

f = 'output/{}_budget.{}_hill_climb_{:%Y%m%d.%H%M}.pkl'.format(
